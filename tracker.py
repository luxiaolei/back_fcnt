...
from utils import gen_sel_maps
...
class TrackerContour(object):
    ...
    contours. 
    """
    def __init__(self, snet_t0_samples=2):
        self.conf_scores = []
        self.arear_in_bbox = []
        self.step = 0
    ...
    def _compute_score(self, loc_list, arear_list, arear_punish = 1.):		
        ...
        scores = scores_dis / (arear_list * arear_punish)
        best_idx = np.argmin(scores)
        return loc_list[best_idx], best_idx, scores[best_idx]


    def _roi_2_img_space_transform(self, loc_roi, rz_factor):
        """Returns a transformed location in image space."""
        x,y,w,h = loc_roi
        dx, dy = (x+(w/2)-112)/rz_factor, (y+(h/2)-112)/rz_factor
        pre_loc = [self.gt_last[0]+dx, self.gt_last[1]+dy, w/rz_factor, h/rz_factor]
        return [abs(int(i)) for i in pre_loc]		
    

    def init_first_img(self, sess, img, roi_t0, vgg, idx_c4, idx_c5, gt_M, gt):
        c4_maps_0, _ = gen_sel_maps(sess, roi_t0, vgg, idx_c4, idx_c5)

        self.targets_records = [gt_M]
        self.c4_maps_records = c4_maps_0

        self.c4_maps_gt = np.repeat(c4_maps_0, self.snet_t0_samples,axis=0)
        self.targets_gt = np.repeat(np.array([gt_M]), self.snet_t0_samples, axis=0)

        x,y,w,h = gt
        self.target_t0_mean = img[y:y+h, x:x+w].mean()
        self.first_roi = roi_t0
    ...
    def adjust_brightness(self, image):
        """
        Returns an image with adjusted target arear's brightness.
        In the way that its mean intensity is the same as the ones 
        in the first frame.

        This trick is particulally usefully in the cases of when 
        target movie into a shadow.
    ...
    def preporcess_roi(self, roi, noisy_cap=None):
		
        if noisy_cap is None: noisy_cap = roi.max()*0.2
        roi[roi<noisy_cap] = noisy_cap
        return roi


    def preporcess_heatmaps(self, pre_M_g=None, pre_M_s=None, uses='s', resize=(224,224)):
        """
        Preporcessing heatmaps generated by SGNets,
        returns a combined, resized, np.float32 type heatmap.

        """
        if uses == 'g' and pre_M_g is not None:
            pre_M_g = imresize(pre_M_g, (224,224)).astype(np.float32)
            return pre_M_g/pre_M_g.max()
        elif uses == 's' and pre_M_s is not None:
            pre_M_s = imresize(pre_M_s, (224,224)).astype(np.float32)
            return pre_M_s/pre_M_s.max()
    ...
    def predict_location(self,
        ...
        pre_M_tmp = pre_M.copy()
        if self.step == 0:
            pre_M_tmp[pre_M_tmp<(0.7)] = 0
        ...
        pre_loc_img = self._roi_2_img_space_transform(pre_loc_roi, rz_factor)
        self.arear_in_bbox += [pre_loc_img[2]*pre_loc_img[3]]
        self.step += 1
        self.gt_last = pre_loc_img
        return pre_loc_img, pre_loc_roi
...
class TrackerVanilla(TrackerPf):
    """Vanilla tracker
    ...
    """
    __doc__ = TrackerPf.__doc__ + __doc__
    def __init__(self, init_location):
        """
        Args:
